...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/05/26 19:27:20 by javperez          #+#    #+#             */
/*   Updated: 2024/08/13 19:49:49 by javperez         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_init_img(t_map *map)
{
	...
	map->img[4].mlx_img = mlx_new_image(map->mlx, //Esto dibuja en negro
			map->display_width, map->display_height);
	if (!map->img[4].mlx_img)
		game_error(map, "Error\nImage initialisation has failed\n");
	i = 0;
	/*	SECTION
		Básicamente mlx_get_data_addr va a manipular los datos al nivel más bajo
	*/
	while (i < 5)
	{
...
void	ft_init_game_stat(t_map *map)
{
	...
	if (map->p_dir == 'N') // map -> p_dir es lo que se le pase por el mapa NSEW
	{
		map->plane_x = 0.66;
		map->plane_y = 0;
	}
...
int	ft_init_game(t_map *map)
{
	...
	/*
		Hasta aqui hemos inicializado valores basicamente ahora toca las mates
	*/
